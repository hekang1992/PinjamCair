...
class WalletViewController: BaseViewController {
    ...
    lazy var tableView: UITableView = {
        ...
        tableView.register(CoachViewCell.self, forCellReuseIdentifier: "CoachViewCell")
        tableView.register(ChanelViewCell.self, forCellReuseIdentifier: "ChanelViewCell")
...
extension WalletViewController {
    ...
    private func getWalletInfo() async {
        do {
            ...
            let model = try await viewModel.getWalletInfo(parameters: parameters)
            let ectopurposeess = model.ectopurposeess ?? ""
            if ["0", "00"].contains(ectopurposeess) {
                self.model = model
...
extension WalletViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let model = self.model?.casia?.die?[indexPath.row]
        let type = model?.scienceile ?? ""
        if type == "successization" {
            let cell = tableView.dequeueReusableCell(withIdentifier: "ChanelViewCell", for: indexPath) as! ChanelViewCell
            cell.model = model
            return cell
        }else {
            let cell = tableView.dequeueReusableCell(withIdentifier: "CoachViewCell", for: indexPath) as! CoachViewCell
            cell.model = model
